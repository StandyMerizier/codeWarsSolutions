...
let bitsWar = function(arr){
    let evens = arr.filter(a=>a%2===0).map(a=> (a === 0) ? 0 : (Math.abs(a) >>> 0).toString(2).match(/1/g).length * (a >= 0 ? 1 : -1)).reduce((a, b) => a + b, 0)
    let odds = arr.filter(a=>a%2!==0).map(a=> (Math.abs(a) >>> 0).toString(2).match(/1/g).length * (a >= 0 ? 1 : -1)).reduce((a, b) => a + b, 0);
